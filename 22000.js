...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 0 && mode == 0) {
			cm.sendOk("흠.. 아직 이곳에서 할 일이 남았나 보지?");
			cm.dispose();
			return;
		}
		if (mode == 1)
		...
		if (status == 0) {
			...
			if(cm.haveItem(4031801,1)){
				lucas = 1;
				cm.sendYesNo("좋아. 그럼 어서 150메소를 줘... 응? 그건 암허스트의 장로 루카스님의 추천서잖아? 뭐야, 이런 게 있었으면 진작 말을 했어야지. 루카스님이 추천할 정도로 재능 있는 모험가에게 돈을 받을 정도로 이 샹크스가 매정하지는 않다고!\r\n추천서를 가지고 있으니, 특별히 요금은 면제해 줄게. 이제 빅토리아 아일랜드로 출발한다! 흔들릴지도 모르니 꽉 잡아!");
			} else {			
			...
			cm.gainMeso(-150);
			// cm.warp(104000000);
			cm.warp(104000000, 0); // 리스항구 0번 포탈 고정
			cm.dispose();
		}
	}
}
