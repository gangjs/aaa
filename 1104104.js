...
function getMapMonster() {
    ...
    if (show == true) {
        ...
        cm.sendOk("이 맵에는 몬스터가 없거나 드롭 아이템이 존재하지 않습니다.");
        cm.dispose();
        return;
    }
}
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var say = " #e[드롭 테이블 검색]#n\r\n";
        say += "#L1##b몬스터 이름으로 검색#k#l\r\n";
        say += "#L2##b아이템 이름으로 검색#k#l\r\n";
        say += "#L3#글로벌 드롭 아이템 확인#l\r\n";
        say += "#L4##b현재 맵 몬스터 확인#l\r\n\r\n";
        cm.sendSimple(say);
    } else if (status == 1) {
        if (selection == 1) {
            choice = 1;
            cm.sendGetText("#e드롭률을 확인 할 몬스터 이름을 검색해 주세요.\r\n#e일부 검색어는 팅김 현상#n이 발생할 수 있습니다.#b");
        } else if (selection == 2) { //아이템 정보를 쿼리에서 불러옴
            choice = 2;
            cm.sendGetText("드롭률을 확인 할 아이템 이름을 검색해 주세요.\r\n#e일부 검색어는 팅김 현상#n이 발생할 수 있습니다.");
        } else if (selection == 3) {
            choice = 2;
            status = 2;
            getAllMonsterDrop();
        ...
        if (choice == 1) {
			if(cm.getText().length() < 1){
				cm.sendOk("검색어를 입력해 주세요.");
				cm.dispose();
				return;
			} 
